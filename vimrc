...
"PluginInstall  PluginUpdate  PluginSearch
...
Plugin 'vim-scripts/nginx.vim'          "nginx语法
...
func SetTitle() 
    if &filetype == 'sh'     "如果文件类型为.sh文件
        ...
        call append(line(".")+4, "\# Created Time: ".strftime("%F_%T"))
        call append(line(".")+5, "\##############################")
        call append(line(".")+6, "") 
    elseif &filetype == 'python'     "如果文件类型为.py文件
        ...
        call append(line(".")+5, "\# Created Time: ".strftime("%F_%T"))
        call append(line(".")+6, "\##############################")
        call append(line(".")+7, "")
    endif
    "新建文件后，自动定位到文件末尾
